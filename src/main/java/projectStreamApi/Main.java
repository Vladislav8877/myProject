...
public class Main {
    public static void main(String[] args) {
        ...
        List<Product> onlyChildrenProduct = customers.stream()
                .flatMap(n -> n.getOrders().stream())
                .flatMap(n -> n.getProducts().stream())
                .filter(n -> n.getCategory().equals("Children's products"))
                .distinct()
        ...
        List<Product> fromFebruaryToApril = customers.stream()
                ...
                .flatMap(n -> n.getOrders().stream())
                //.peek(n -> System.out.println("Заказы покупателей 2>=lvl: "+n))
                .filter(n -> !n.getOrderDate().isBefore(startDate)
                        && !n.getDeliveryDate().isAfter(endDate))
                .flatMap(n -> n.getProducts().stream())
                .toList();
        ...
        List<Product> fifteenthOfMarch = customers.stream()
                .flatMap(n -> n.getOrders().stream())
                .filter(n -> n.getOrderDate().equals(targetDate))
                .distinct()
                .peek(n -> System.out.println("Найден заказ от: " + n.getOrderDate() + " || ID: " + n.getId()))
                .flatMap(n -> n.getProducts().stream())
                .distinct()
                .sorted(Comparator.comparing(Product::getId))
                .toList();
        ...
        BigDecimal mayPrice = customers.stream()
                .flatMap(n -> n.getOrders().stream())
                .filter(n -> n.getOrderDate().getMonth() == Month.FEBRUARY)
                .flatMap(n -> n.getProducts().stream())
        ...
        List<BigDecimal> midlPriceForMarch = customers.stream()
                .flatMap(n -> n.getOrders().stream())
                .filter(n -> n.getOrderDate().equals(LocalDate.of(2021, 3, 14)))
                .distinct()
                .flatMap(n -> n.getProducts().stream())
                .map(Product::getPrice)
                .toList();
        ...
        System.out.printf("Кол-во проданных книг: %d%n", refreshBook.getCount());
        System.out.printf("Общая сумма выручки: %.2f%n", refreshBook.getSum());
        System.out.printf("Средний чек за книгу: %.2f%n", refreshBook.getAverage());
        System.out.printf("Самая дорогая книга: %.2f%n", refreshBook.getMax());
        System.out.printf("Самая дешевая книга: %.2f%n", refreshBook.getMin());
        System.out.println("-----------------");
